...
int main(int argc, char** argv) {
    ...
    dut->trace(tfp, 99);
    tfp->open("sha256_1mb_waveform.vcd");
    
    // Simulation variables
    vluint64_t sim_time = 0;
    const vluint64_t max_sim_time = 10000000;  // Increased to 10M time units for full 1MB processing
    
    std::cout << "=== SHA256 1MB Processor Simulation ===" << std::endl;
    ...
    while (sim_time < max_sim_time && !Verilated::gotFinish()) {
        ...
        if (dut->clk) {  // Rising edge
            ...
            // Progress indication every 5000 cycles with actual block count
            if (test_started && !test_completed && (cycles_since_start - last_progress_report) >= 5000) {
                last_progress_report = cycles_since_start;
                
                // Try to read the actual block count from the processor
                ...
                if (cycles_since_start > 500000) {  // After reasonable time
    ...
    if (test_completed) {
        ...
        std::cout << "Try increasing max_sim_time if needed" << std::endl;
        std::cout << "Current timeout: " << max_sim_time << " time units" << std::endl;
        std::cout << "Suggested timeout for full completion: ~1,000,000 time units" << std::endl;
    }

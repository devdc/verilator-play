...
int main(int argc, char** argv) {
    ...
    Vsha256_1kb_processor_fixed* dut = new Vsha256_1kb_processor_fixed;
    
    // Initialize VCD tracing
    Verilated::traceEverOn(true);
    VerilatedVcdC* tfp = new VerilatedVcdC;
    dut->trace(tfp, 99);
    tfp->open("sha256_1kb_fixed_waveform.vcd");
    
    // Simulation variables
    vluint64_t sim_time = 0;
    const vluint64_t max_sim_time = 50000;  // Reduced maximum simulation time
    
    std::cout << "=== SHA256 1KB Processor Fixed Simulation ===" << std::endl;
    ...
    while (sim_time < max_sim_time && !Verilated::gotFinish()) {
        ...
        if (dut->clk) {  // Rising edge
            ...
            if (dut->done && !test_completed) {
                ...
                std::cout << std::endl;
                std::cout << "Expected: 785b0751fc2c53dc14a4ce3d800e69ef9ce1009eb327ccf458afe09c242c26c9" << std::endl;
